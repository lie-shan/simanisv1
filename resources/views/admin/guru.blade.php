<x-admin-layout>
    ...
    <style>
        ...
        table.guru-table thead { background: #fff; }
        table.guru-table th {
            text-align: center;
            font-size: 11px;
            font-weight: 700;
        ...
        table.guru-table td:nth-child(2) { text-align: left; }
        table.guru-table tbody tr:hover { background: #f8faff; }
        table.guru-table tbody tr:last-child td { border-bottom: none; }
        .guru-nama {
            display: flex;
            align-items: center;
            gap: 12px;
        ...
        .guru-name-text { font-weight: 600; color: #2c3e50; }
        .guru-no-registrasi-text { font-size: 11px; color: var(--text-muted); margin-top: 1px; }
        .badge-status {
            display: inline-flex;
            align-items: center;
            gap: 5px;
        ...
        .toast-close {
            ...
            display: flex; align-items: center; justify-content: center;
            font-size: 20px; color: #999; flex-shrink: 0; transition: all 0.15s;
        }
        ...
        .modal-header h3 { font-size: 16px; font-weight: 700; color: #333; margin: 0; }
        .modal-close {
            width: 32px; height: 32px; border-radius: 8px; border: none;
            background: #f0f0f0; cursor: pointer;
            display: flex; align-items: center; justify-content: center;
            font-size: 16px; color: #666; transition: all 0.15s;
        }
        ...
        .modal-footer {
            ...
            gap: 8px; padding: 16px 20px; border-top: 1px solid var(--border-color);
        }
        ...
        @media (max-width: 575px) {
            ...
            table.guru-table th, table.guru-table td { padding: 11px 12px; font-size: 12px; }
    ...
    <div class="stats-row">
        <div class="stat-box">
            <div class="stat-box-icon purple"><i class="fa-solid fa-chalkboard-user"></i></div>
            <div class="stat-box-body">
                <div class="stat-box-label">Total Guru</div>
                <div class="stat-box-value">{{ $guru->count() }}</div>
            </div>
        </div>
        <div class="stat-box">
            ...
            <div class="stat-box-body">
                ...
                <div class="stat-box-value">{{ $guru->where('jk', 'L')->count() }}</div>
            </div>
        </div>
        <div class="stat-box">
            ...
            <div class="stat-box-body">
                ...
                <div class="stat-box-value">{{ $guru->where('jk', 'P')->count() }}</div>
            </div>
        </div>
        <div class="stat-box">
            <div class="stat-box-icon teal"><i class="fa-solid fa-graduation-cap"></i></div>
            <div class="stat-box-body">
                <div class="stat-box-label">Aktif</div>
                <div class="stat-box-value">{{ $guru->where('status', 'Aktif')->count() }}</div>
            </div>
        </div>
        <div class="stat-box">
            <div class="stat-box-icon" style="background:linear-gradient(135deg,#eef2ff,#e0e7ff);color:#4f46e5;"><i class="fa-solid fa-people-group"></i></div>
            <div class="stat-box-body">
                <div class="stat-box-label">Wali Kelas</div>
                <div class="stat-box-value">{{ $kelasData->whereNotNull('wali_kelas')->count() }}</div>
            </div>
        </div>
    </div>
        ...
        <div class="filter-top">
            <div class="filter-left">
                <div class="search-box">
                    ...
                    <input type="text" id="filterSearch" placeholder="Cari guru..." oninput="filterTable()">
                </div>
            ...
            <div class="filter-right">
                ...
                <button class="btn fill" style="padding:8px 18px;" onclick="openCreateModal()"><i class="fa-solid fa-plus"></i> Tambah Guru</button>
            </div>
        </div>
    </div>
    ...
    <div class="table-wrap">
        <div class="table-header">
            ...
            <div style="display:flex;gap:8px;">
                ...
                <a href="{{ route('admin.guru.export') }}" class="filter-btn ghost"><i class="fa-solid fa-download"></i> Export</a>
            </div>
        </div>
        ...
        <div class="table-scroll">
            <table class="guru-table">
                ...
                <tbody>
                    @foreach($guru as $i => $g)
                    @php
                        $guruKelas = $kelasData->firstWhere('wali_kelas', $g['nama']);
                    @endphp
                    <tr>
                        <td style="color:var(--text-muted);font-weight:500;">{{ $i + 1 }}</td>
                        <td>
                            <div class="guru-nama">
                                <div class="guru-avatar" style="background: {{ ['#0d6efd','#6f42c1','#d63384','#dc3545','#fd7e14','#ffc107','#198754','#0dcaf0','#6610f2','#e83e8c','#20c997','#17a2b8'][$i % 12] }}">
                                    @if($g['foto'])
                                        <img src="{{ asset('storage/'.$g['foto']) }}" style="width:100%;height:100%;object-fit:cover;">
                                    @else
                                        {{ substr($g['nama'], 0, 1) }}
                                    @endif
                                </div>
                                <div>
                                    <div class="guru-name-text">{{ $g['nama'] }}</div>
                                    <div class="guru-no-registrasi-text">No. Registrasi: {{ $g['no_registrasi'] }}</div>
                                </div>
                            </div>
                        </td>
                        <td>
                            @if($guruKelas)
                                <span class="kelas-tag">{{ $guruKelas->nama_kelas }}</span>
                            @else
                        ...
                        <td>{{ $g['jk'] }}</td>
                        <td>{{ $g['tmp_lahir'] }}</td>
                        <td>@php try { echo \Carbon\Carbon::parse($g['tgl_lahir'])->locale('id')->isoFormat('D MMM YYYY'); } catch (\Exception $e) { echo $g['tgl_lahir']; } @endphp</td>
                        <td>{{ $g['no_hp'] ?? '-' }}</td>
                        <td>
                            <span class="badge-status {{ strtolower(str_replace(' ', '-', $g['status'])) }}">
                                <span class="dot"></span> {{ $g['status'] }}
                            </span>
                        </td>
                        <td>
                            <div class="action-btns">
                                <button class="action-btn view" title="Detail" onclick="openDetailModal({{ $g['id'] }})"><i class="fa-regular fa-eye"></i></button>
                                <button class="action-btn edit" title="Edit" onclick="openEditModal({{ $g['id'] }})"><i class="fa-regular fa-pen-to-square"></i></button>
                                <button class="action-btn delete" title="Hapus" onclick="confirmDelete({{ $g['id'] }})"><i class="fa-regular fa-trash-can"></i></button>
                            </div>
                        </td>
                    </tr>
                    @endforeach
        ...
        <div class="pagination-wrap">
            <div class="pagination-info">Menampilkan 1-{{ $guru->count() }} dari {{ $guru->count() }} guru</div>
        ...
        <div class="modal-box" style="max-width:440px;">
            ...
            <form action="{{ route('admin.guru.import') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body" style="text-align:center;padding:24px;">
                    ...
                    <a href="{{ route('admin.guru.template') }}" class="filter-btn ghost" style="font-size:12px;padding:6px 14px;"><i class="fa-solid fa-download"></i> Download Template</a>
                </div>
            ...
            <form id="editForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label>No. Registrasi</label>
                            <input type="text" id="edit_no_registrasi" class="form-control" disabled style="color:#999;">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="edit_wali_kelas" class="form-control" disabled style="color:#4f46e5;background:#eef2ff;font-weight:600;">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
    ...
    <div class="modal-overlay" id="deleteModal">
        <div class="modal-box" style="max-width:400px;">
            <div class="modal-body" style="text-align:center;padding:30px 24px 20px;">
                ...
                <h3 style="font-size:17px;font-weight:700;color:#1a1a2e;margin:0 0 6px;">Hapus Guru</h3>
                <p style="font-size:13px;color:#666;margin:0 0 20px;line-height:1.5;">Apakah Anda yakin ingin menghapus guru ini?<br>Tindakan ini tidak dapat dibatalkan.</p>
    ...
    <script>
        ...
        function filterTable() {
            ...
            var rows = document.querySelectorAll('.guru-table tbody tr');
            rows.forEach(function(row) {
                var text = row.textContent.toLowerCase();
                var match = true;
                if (search && text.indexOf(search) === -1) match = false;
                if (kelas) {
                    var kelasVal = row.querySelector('td:nth-child(3)').textContent.trim();
                    if (kelasVal !== kelas) match = false;
                }
                if (jk) {
        ...
        function openEditModal(id) {
            fetch('/admin/guru/' + id + '/edit')
                ...
                .then(data => {
                    ...
                    var waliKelas = @json($kelasData->keyBy('wali_kelas')->map(function($k) { return $k->nama_kelas; }));
                    document.getElementById('edit_wali_kelas').value = waliKelas[data.nama] || '-';
        ...
        function openDetailModal(id) {
            fetch('/admin/guru/' + id + '/edit')
                .then(r => r.json())
                .then(data => {
                    fetch('/admin/kelas?wali=' + encodeURIComponent(data.nama))
                        .then(function(r) { return r.json ? r : r.json(); })
                        .catch(function() { return null; });
                    var kelasName = '';
                    var kelasData = @json($kelasData->keyBy('wali_kelas')->map(function($k) { return $k->nama_kelas; }));
                    if (kelasData[data.nama]) {
                        kelasName = kelasData[data.nama];
                    }
                    var fotoHtml = '';
                    if (data.foto) {
                    ...
                    html += '<div style="display:flex;gap:10px;margin-bottom:10px;">'+dl('Wali Kelas', kelasName ? '<span style="display:inline-flex;align-items:center;gap:4px;padding:2px 10px;border-radius:6px;font-size:11px;font-weight:600;background:#eef2ff;color:#4f46e5;">'+kelasName+'</span>' : '<span style="color:#94a3b8;">-</span>')+dl('Jenis Kelamin', data.jk == 'L' ? 'Laki-laki' : 'Perempuan')+'</div>';
        ...
        function submitDelete() {
            if (deleteId) {
                document.getElementById('deleteForm').action = '/admin/guru/' + deleteId;
